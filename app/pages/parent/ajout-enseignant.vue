      ...
      <div class="p-8 h-full flex flex-col">
        ...
        <div class="flex items-center gap-3 mb-12 group cursor-pointer" @click="goToDashboard">
          ...
          <div>
            ...
            <p :class="isDark ? 'text-purple-300' : 'text-purple-600'" class="text-sm font-medium mt-1">Création Enseignant</p>
          </div>
        </div>
        ...
        <nav class="pt-6" :class="isDark ? 'border-purple-400/20' : 'border-purple-400/30'">
          <div :class="isDark ? 'bg-slate-800/30 border-white/10' : 'bg-white/30 border-gray-300/30'" class="p-4 rounded-2xl border shadow-2xl backdrop-blur-xl transition-colors duration-500">
            ...
            >
              ...
              <span :class="isDark ? 'text-white group-hover:text-blue-200' : 'text-gray-800 group-hover:text-blue-600'" class="font-bold transition-colors duration-300 relative z-10">Créer Enseignant</span>
        ...
        <div :class="isDark ? 'bg-slate-800/20 border-white/10' : 'bg-white/20 border-gray-300/30'" class="backdrop-blur-3xl rounded-3xl border shadow-2xl overflow-hidden relative transition-colors duration-500">
          ...
          <div class="p-6 relative z-10">
            <form @submit.prevent="handleSubmit" class="space-y-4">
              ...
              <!-- Email -->
              <div class="space-y-2">
                <label :class="isDark ? 'text-purple-300' : 'text-purple-600'" class="block text-sm font-bold flex items-center gap-2">
                  <i :class="isDark ? 'text-purple-400' : 'text-purple-500'" class="fas fa-envelope text-xs"></i>
                  Email *
                </label>
                <input 
                  ...
                  :class="isDark ? 'bg-slate-700/30 border-white/10 text-white placeholder-slate-400 focus:ring-purple-500' : 'bg-white/50 border-gray-300/50 text-gray-800 placeholder-gray-500 focus:ring-purple-400'"
                  class="w-full px-3 py-2 rounded-xl focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300 shadow-lg text-sm backdrop-blur-sm"
                >
              </div>

              <!-- COMMENTÉ: Numéro de téléphone -->
              <!-- <div class="space-y-2">
                <label :class="isDark ? 'text-cyan-300' : 'text-cyan-600'" class="block text-sm font-bold flex items-center gap-2">
                  <i :class="isDark ? 'text-cyan-400' : 'text-cyan-500'" class="fas fa-phone text-xs"></i>
                  Téléphone *
                </label>
                <div class="relative">
                  <span :class="isDark ? 'text-cyan-400' : 'text-cyan-500'" class="absolute left-3 top-1/2 transform -translate-y-1/2 font-bold text-sm flex items-center gap-1">
                    <i :class="isDark ? 'text-cyan-400' : 'text-cyan-500'" class="fas fa-flag text-xs"></i>
                    +228
                  </span>
                  <input 
                    ...
                    class="w-full px-16 py-2 rounded-xl focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300 shadow-lg text-sm backdrop-blur-sm"
                    @input="formatPhoneNumber"
                  >
                </div>
                <p :class="isDark ? 'text-cyan-400' : 'text-cyan-600'" class="text-xs flex items-center gap-1">
                  <i class="fas fa-info-circle text-xs"></i>
                  Format: 00 00 00 00 (8 chiffres)
                </p>
              </div> -->

              <!-- Mode de paiement -->
              <div class="space-y-2">
                <label :class="isDark ? 'text-indigo-300' : 'text-indigo-600'" class="block text-sm font-bold flex items-center gap-2">
                  <i :class="isDark ? 'text-indigo-400' : 'text-indigo-500'" class="fas fa-credit-card text-xs"></i>
                  Mode de paiement *
                </label>
                <select 
                  v-model="form.mode_paiement" 
                  required
                  :class="isDark ? 'bg-slate-700/30 border-white/10 text-white placeholder-slate-400 focus:ring-indigo-500' : 'bg-white/50 border-gray-300/50 text-gray-800 placeholder-gray-500 focus:ring-indigo-400'"
                  class="w-full px-3 py-2 rounded-xl focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300 shadow-lg text-sm backdrop-blur-sm"
                >
                  <option disabled value="">Choisir le mode</option>
              ...
              <!-- Salaire -->
              <div class="space-y-2">
                <label :class="isDark ? 'text-green-300' : 'text-green-600'" class="block text-sm font-bold flex items-center gap-2">
                  <i :class="isDark ? 'text-green-400' : 'text-green-500'" class="fas fa-money-bill-wave text-xs"></i>
                  Salaire (FCFA) *
                </label>
                <div class="relative">
                  <span :class="isDark ? 'text-green-400' : 'text-green-500'" class="absolute left-3 top-1/2 transform -translate-y-1/2 font-bold text-sm">FCFA</span>
                  <input 
                    ...
                    min="0"
                    placeholder="0"
                    :class="isDark ? 'bg-slate-700/30 border-white/10 text-white placeholder-slate-400 focus:ring-green-500' : 'bg-white/50 border-gray-300/50 text-gray-800 placeholder-gray-500 focus:ring-green-400'"
                    class="w-full px-12 py-2 rounded-xl focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300 shadow-lg text-right font-bold text-sm backdrop-blur-sm"
                  >
                </div>
              </div>
...
// COMMENTÉ: Formatage du numéro de téléphone
...
//   form.value.telephone = value
// }

// COMMENTÉ: Validation du numéro de téléphone
// const validatePhoneNumber = (phone) => {
//   const phoneDigits = phone.replace(/\s/g, '') // Supprimer les espaces
//   return phoneDigits.length === 8 && /^0[1-8]\d{7}$/.test(phoneDigits)
// }
...
const handleSubmit = async () => {
  ...
  if (!form.value.nom_famille || !form.value.prenom || !form.value.courriel || !form.value.mode_paiement || !form.value.salaire) {
  ...
  // COMMENTÉ: Validation téléphone
  // if (!validatePhoneNumber(form.value.telephone)) {
  //   showMessage('Veuillez entrer un numéro de téléphone valide (8 chiffres, format: 00 00 00 00)', 'error')
  //   return
  // }
  ...
  try {
    ...
    // Préparer les données (sans le champ téléphone)
    const submissionData = {
      nom_famille: form.value.nom_famille,
      prenom: form.value.prenom,
      courriel: form.value.courriel,
      // COMMENTÉ: telephone: form.value.telephone.replace(/\s/g, ''), // Supprimer les espaces pour l'API
    ...
    showMessage(' Enseignant créé avec succès ! Redirection...', 'success')
    ...
    setTimeout(() => {
      router.push('/parent/create-enseignant')
    }, 2000)
