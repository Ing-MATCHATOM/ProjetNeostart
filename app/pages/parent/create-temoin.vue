        ...
        <table class="w-full table-auto border-collapse border border-gray-200">
          ...
          <tbody>
            <tr v-for="temoin in temoins" :key="temoin.id" class="hover:bg-gray-50">
              <td class="border px-4 py-2">{{ temoin.temoin.nom }}</td>
              <td class="border px-4 py-2">{{ temoin.temoin.courriel }}</td>
      ...
      <div v-else class="max-w-md mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="p-6">
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <input v-model="form.courriel" type="email" placeholder="Email" class="input" required />
...
const temoins = ref([])
const form = ref({ nom: '', courriel: '' })
const loading = ref(false)

// Récupérer la liste des témoins
...
const handleSubmit = async () => {
  ...
  try {
    ...
    alert('Témoin créé avec succès')
    form.value = { nom_famille: '', mail: '' }
    showList.value = true
    fetchTemoins() // rafraîchir la liste
